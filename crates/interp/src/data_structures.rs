use crate::{read_scriptint, ExecError, MAX_SCRIPT_ELEMENT_SIZE};
use alloc::rc::Rc;
...
impl StackEntry {
    /// Script-encoded bytes of this entry. Upstream asserted a 4-byte limit
    /// here, which holds for BitVM's u32 arithmetic but not for Bitcoin
    /// script in general: a template hash, signature, or public key on the
    /// stack would abort. Entries are returned at their real length.
...
impl<'de> Visitor<'de> for StackEntryVisitor {
        ...
        E: de::Error,
    {
        if v.len() > MAX_SCRIPT_ELEMENT_SIZE {
            return Err(de::Error::invalid_value(Unexpected::Bytes(v), &self));
        }
        ...
        A: de::SeqAccess<'de>,
    {
        ...
        if vec.len() > MAX_SCRIPT_ELEMENT_SIZE {
...
impl Stack {
    ...
    pub fn iter_str(&self) -> Map<Iter<'_, StackEntry>, fn(&StackEntry) -> Vec<u8>> {
