...
export interface MenuItem {
  ...
  separator?: boolean; heading?: boolean; detail?: string;
...
function Row({ it, className, ...rest }: { it: MenuItem; className: string } & ComponentProps<"button">) {
  return (
    <button className={className} role="menuitem" disabled={it.disabled} tabIndex={-1} title={it.label}
      aria-haspopup={it.submenu ? "menu" : undefined} {...rest}>
      {it.checked !== undefined && <i className="menu-tick" aria-hidden="true">{it.checked ? "✓" : ""}</i>}
      <span>{it.label}</span>
      {it.shortcut && <kbd>{it.shortcut}</kbd>}
      {it.detail && <em>{it.detail}</em>}
      {it.submenu && <i className="menu-more" aria-hidden="true">›</i>}
    </button>
  );
}

export function ContextMenu({ x, y, items, onClose, ignore }: {
  ...
  const setCursor = (i: number) => { cursorRef.current = i; setCursorState(i); };
  ...
  const setSub = (v: { at: number; x: number; y: number } | null) => { subRef.current = v; setSubState(v); };
  const [subCursor, setSubCursorState] = useState(-1);
  const subCursorRef = useRef(-1);
  const setSubCursor = (i: number) => { subCursorRef.current = i; setSubCursorState(i); };
  ...
  const openSub = (at: number) => {
    ...
    const w = 300, h = Math.min(list.length * 30 + 12, window.innerHeight - 16);
  ...
  useEffect(() => {
    ...
    const step = (list: { i: number }[], at: number, key: string) =>
      at < 0 ? (key === "ArrowDown" ? 0 : list.length - 1)
        : key === "ArrowDown" ? (at + 1) % list.length : (at - 1 + list.length) % list.length;
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape") { e.preventDefault(); if (subRef.current) setSub(null); else onClose(); return; }
      ...
      if (subRef.current) {
        ...
        if (e.key === "ArrowDown" || e.key === "ArrowUp") {
          ...
          setSubCursor(inner[step(inner, inner.findIndex(({ i }) => i === subCursorRef.current), e.key)].i);
          return;
        }
        if (e.key === "Enter" || e.key === " ") {
          const hit = inner.find(({ i }) => i === subCursorRef.current);
          if (hit) { e.preventDefault(); hit.it.onClick?.(); onClose(); }
          return;
        }
        return;
      }
      if (e.key === "ArrowDown" || e.key === "ArrowUp") {
        ...
        setCursor(top[step(top, top.findIndex(({ i }) => i === cursorRef.current), e.key)].i);
        return;
      }
      if (e.key === "ArrowRight") {
        const hit = top.find(({ i }) => i === cursorRef.current);
        if (hit?.it.submenu) { e.preventDefault(); openSub(hit.i); }
        return;
      }
      if (e.key === "Enter" || e.key === " ") {
        const hit = top.find(({ i }) => i === cursorRef.current);
        if (!hit) return;
        e.preventDefault();
        if (hit.it.submenu) { openSub(hit.i); return; }
        hit.it.onClick?.();
        onClose();
      }
    };
  ...
  useEffect(() => { if (cursorRef.current >= 0 && !top.some(({ i }) => i === cursorRef.current)) setCursor(-1); }, [top]);
  ...
  // not 30px tall.
  const [pos, setPos] = useState(() => clampToViewport(x, y, 240, Math.min(items.length * 30 + 12, window.innerHeight - 16)));
  useLayoutEffect(() => {
    const r = ref.current?.getBoundingClientRect();
    if (r) setPos(clampToViewport(x, y, r.width, r.height));
  }, [x, y, items.length]);
  return (
    <div className="menu" ref={ref} style={{ left: pos.x, top: pos.y }} role="menu" tabIndex={-1}
      aria-activedescendant={cursor >= 0 ? `${uid}-${cursor}` : undefined}>
      {items.map((it, i) => it.separator ? <div className="menu-sep" key={i} /> : it.heading ? <div className="menu-cat" key={i}>{it.label}</div> : (
        <Row key={i} it={it} id={`${uid}-${i}`} className={`menu-item ${i === cursor ? "cur" : ""}`}
          ref={(el: HTMLButtonElement | null) => { rows.current[i] = el; }}
          aria-expanded={it.submenu ? sub?.at === i : undefined}
          onMouseEnter={() => { setCursor(i); if (it.submenu) openSub(i); else setSub(null); }}
          onClick={() => { if (it.submenu) { openSub(i); return; } it.onClick?.(); onClose(); }} />
      ))}
      ...
      {sub && (
        <div className="menu menu-sub" role="menu" style={{ left: sub.x, top: sub.y }}>
          {kids.map((it, j) => it.separator ? <div className="menu-sep" key={j} /> : it.heading ? <div className="menu-cat" key={j}>{it.label}</div> : (
            <Row key={j} it={it} className={`menu-item ${j === subCursor ? "cur" : ""}`}
...
export function AddMenu({ x, y, pendingPort, pendingSide, onPick, onClose }: AddMenuProps) {
  ...
  const items = useMemo(() => {
    ...
    return Object.values(KINDS)
      ...
      .filter((k) => !needle || k.label.toLowerCase().includes(needle) || k.description.toLowerCase().includes(needle) || k.category.toLowerCase().includes(needle))
      .map((k) => {
        const d = k.defaults();
        const pin = pendingPort == null ? undefined
          : pendingSide === "source" ? firstCompatibleInput({ ...d, kind: k.kind }, pendingPort)
          : firstCompatibleOutput({ ...d, kind: k.kind }, pendingPort);
        return { kind: k, pin, ok: pendingPort == null || !!pin };
      })
      .sort((a, b) => Number(b.ok) - Number(a.ok) || CATEGORIES.indexOf(a.kind.category) - CATEGORIES.indexOf(b.kind.category));
  ...
  return (
    ...
    <div className="menu add" ref={box} style={{ left: pos.x, top: pos.y }}
      onMouseDown={(e) => { if (!(e.target instanceof HTMLElement) || !e.target.closest("input,button")) e.preventDefault(); }}>
      <input ref={input} className="menu-search" placeholder={pendingPort ? `Add a node to connect ${pendingPort.type}…` : "Add node…"} value={q}
        onChange={(e) => setQ(e.target.value)}
        onKeyDown={(e) => {
          if (e.key === "ArrowDown") { e.preventDefault(); setCursor((c) => Math.min(items.length - 1, c + 1)); }
          if (e.key === "ArrowUp") { e.preventDefault(); setCursor((c) => Math.max(0, c - 1)); }
          if (e.key === "Enter" && items[cursor]?.ok) { e.preventDefault(); onPick(items[cursor].kind.kind); }
          ...
          return (
            <div key={cat}>
              ...
              {group.map((it) => {
                ...
                return (
                  <button key={it.kind.kind} className={`menu-item ${idx === cursor ? "cur" : ""} ${it.ok ? "" : "off"}`} disabled={!it.ok}
                    onMouseEnter={() => setCursor(idx)} onClick={() => it.ok && onPick(it.kind.kind)}>
