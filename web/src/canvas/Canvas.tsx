...
import {
  ReactFlow, Background, BackgroundVariant, MiniMap, SelectionMode, useReactFlow, useViewport, useNodesInitialized,
  type NodeTypes, type Edge, type OnConnectEnd, type OnReconnect,
} from "@xyflow/react";
...
export function Canvas() {
  ...
  useEffect(() => {
    ...
    if (viewAll.current) {
      ...
      viewAll.current = false; rf.fitView({ padding: 0.04, duration: 0 });
  ...
  const [pendingWire, setPendingWire] = useState<{ x1: number; y1: number; x2: number; y2: number; type: string; side: "source" | "target" } | null>(null);
  ...
  const snap = (p: { x: number; y: number }) => ({ x: Math.round(p.x / GRID) * GRID, y: Math.round(p.y / GRID) * GRID });
  const openAdd = useCallback((x: number, y: number, pending?: Pending) => setMenu({ kind: "add", x, y, flow: flowAt(x, y), pending }), [flowAt]);

  // --- placing a node attached to the cursor ---------------------------------
  const [ghost, setGhost] = useState<{ x: number; y: number } | null>(null);
  useEffect(() => { if (!placing) setGhost(null); }, [placing]);
  ...
  const onConnectEnd: OnConnectEnd = useCallback((event, state) => {
    ...
    const el = wrapper.current?.querySelector<HTMLElement>(`.react-flow__node[data-id="${CSS.escape(state.fromNode.id)}"] .react-flow__handle[data-handleid="${CSS.escape(handleId)}"]`);
    ...
    if (el && box) {
      ...
      setPendingWire({ x1: r.left + r.width / 2 - box.left, y1: r.top + r.height / 2 - box.top, x2: me.clientX - box.left, y2: me.clientY - box.top, type, side });
    }
    openAdd(me.clientX, me.clientY, { nodeId: state.fromNode.id, handleId, handleType: side });
  }, [openAdd, s]);
  const closeMenu = useCallback(() => { setMenu(null); setPendingWire(null); }, []);

  // --- reconnect: drag a wire end to another pin, or into space to delete ---
  const onReconnect: OnReconnect = useCallback((oldEdge, c) => { reconnected.current = true; s().reconnect(oldEdge, c); }, [s]);
  const onReconnectStart = useCallback(() => { reconnected.current = false; }, []);
  const onReconnectEnd = useCallback((_: unknown, edge: Edge) => { if (!reconnected.current) s().removeEdge(edge.id); }, [s]);

  // --- splice a dragged node into a wire; carry a comment's contents -------
  const sampleEdges = useCallback(() => {
    const map = new Map<string, { x: number; y: number }[]>();
    for (const e of rawEdges) {
      const path = wrapper.current?.querySelector<SVGPathElement>(`.react-flow__edge[data-id="${CSS.escape(e.id)}"] .react-flow__edge-path`);
      if (!path) continue;
      const len = path.getTotalLength();
      const pts: { x: number; y: number }[] = [];
      for (let t = 0; t <= len; t += 14) { const p = path.getPointAtLength(t); pts.push({ x: p.x, y: p.y }); }
      map.set(e.id, pts);
    }
    return map;
  }, [rawEdges]);
  ...
  const nearestEdge = useCallback((node: FlowNode): string | null => {
    ...
    const w = node.measured?.width ?? 288, h = node.measured?.height ?? 200;
  ...
  const onNodeDragStart = useCallback((_: unknown, node: FlowNode) => {
    ...
    edgePts.current = node.data.kind === "comment" || node.data.kind === "reroute" || connected ? null : sampleEdges();
    ...
    const w = Number(node.data.width), h = Number(node.data.height);
    const sizeOf = (n: FlowNode) => n.data.kind === "comment" ? { w: Number(n.data.width), h: Number(n.data.height) } : { w: n.measured?.width ?? 288, h: n.measured?.height ?? 200 };
    const inside = s().nodes.filter((n) => {
      if (n.id === node.id || n.selected) return false;
      const sz = sizeOf(n);
      return n.position.x >= node.position.x && n.position.y >= node.position.y
        && n.position.x + sz.w <= node.position.x + w && n.position.y + sz.h <= node.position.y + h;
    }).map((n) => n.id);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (typing) return;
      if (e.key === "Escape") { if (menu) closeMenu(); else if (s().placing) s().setPlacing(null); else s().deselectAll(); return; }
      ...
      if (mod && e.key.toLowerCase() === "v") { e.preventDefault(); s().paste(snap(flowAt(mouse.current.x, mouse.current.y))); return; }
      if (mod && e.key.toLowerCase() === "d") { e.preventDefault(); s().duplicate(); return; }
      if (mod && e.key.toLowerCase() === "a") { e.preventDefault(); s().selectAll(); return; }
      if (mod) return;
      if (e.key === "Tab" || (e.shiftKey && e.key.toLowerCase() === "a")) { e.preventDefault(); openAdd(mouse.current.x, mouse.current.y); return; }
      if (e.key === "Delete" || e.key === "Backspace" || e.key.toLowerCase() === "x") { e.preventDefault(); s().removeSelected(); return; }
      if (e.key.toLowerCase() === "f") { const sel = s().nodes.filter((n) => n.selected); rf.fitView({ nodes: sel.length ? sel : undefined, padding: 0.2, duration: 250, maxZoom: 1.25 }); return; }
      if (e.key === "Home") { rf.fitView({ padding: 0.12, duration: 250 }); return; }
      if (e.key.toLowerCase() === "c" && !e.shiftKey) { s().commentAroundSelection(snap(flowAt(mouse.current.x, mouse.current.y))); return; }
      if (e.key === "F2") { const sel = s().selected; if (sel) s().setRenaming(sel); return; }
    };
  ...
  const nodeMenu = (id: string): MenuItem[] => {
    ...
    return [
      ...
      { label: n?.data.collapsed ? "Expand" : "Collapse", onClick: () => s().setField(id, "collapsed", !n?.data.collapsed), disabled: kind?.kind === "reroute" || kind?.kind === "comment" },
      { separator: true, label: "" },
      { label: "Duplicate", shortcut: "⌘D", onClick: () => { s().select(id); s().duplicate(); } },
      { label: "Copy", shortcut: "⌘C", onClick: () => { s().select(id); s().copy(); } },
      { label: "Break all wires", onClick: () => s().breakWires(id), disabled: !s().edges.some((e) => e.source === id || e.target === id) },
      { separator: true, label: "" },
      { label: "Delete", shortcut: "⌫", onClick: () => { s().select(id); s().removeSelected(); } },
    ];
  };
  ...
  const pinMenuItems = (m: NonNullable<typeof pinMenu>): MenuItem[] => {
    ...
    const wires = s().edges.filter((e) => side === "source" ? e.source === nodeId && e.sourceHandle === handleId : e.target === nodeId && e.targetHandle === handleId);
    const nameOf = (id: string) => { const n = s().nodes.find((n) => n.id === id); return n ? (n.data.kind === "reroute" ? "knot" : String(n.data.name || n.data.kind)) : id; };
    const portOf = (id: string, handle: string | null | undefined, which: "source" | "target") => { const n = s().nodes.find((n) => n.id === id); return (n && findPort(n.data, which, handle ?? "")?.label) || handle || ""; };
    const isKnot = (id: string) => s().nodes.find((n) => n.id === id)?.data.kind === "reroute";
    const other = (e: Edge) => side === "source"
      ? (isKnot(e.target) ? "knot" : `${nameOf(e.target)} · ${portOf(e.target, e.targetHandle, "target")}`)
      : (isKnot(e.source) ? "knot" : `${nameOf(e.source)} · ${portOf(e.source, e.sourceHandle, "source")}`);
    const items: MenuItem[] = [];
    if (wires.length === 1) items.push({ label: `Break link to ${other(wires[0])}`, shortcut: "alt-click", onClick: () => s().removeEdge(wires[0].id) });
    if (wires.length > 1) {
      items.push({ label: `Break all ${wires.length} links`, shortcut: "alt-click", onClick: () => s().breakWires(nodeId, handleId) });
    ...
    items.push({ label: side === "target" ? "Add a node feeding this pin…" : "Add a node fed by this pin…", onClick: () => setTimeout(() => {
      const n = s().nodes.find((n) => n.id === nodeId);
      const el = document.querySelector(`[data-id="${nodeId}"] [data-handleid="${handleId}"]`) as HTMLElement | null;
      const r = el?.getBoundingClientRect();
      const at = r ? { x: side === "source" ? r.right + 40 : r.left - 40, y: r.top + r.height / 2 } : { x: m.x, y: m.y };
  ...
  const pendingPort = menu?.kind === "add" && menu.pending ? (() => {
  ...
  return (
    <div className={`cv-wrap ${placing ? "placing" : ""}`} ref={wrapper}
      onMouseMove={(e) => { mouse.current = { x: e.clientX, y: e.clientY }; if (placing) setGhost({ x: e.clientX, y: e.clientY }); }}
      onMouseLeave={() => setGhost(null)}>
      <ReactFlow
        nodes={flowNodes} edges={edges} nodeTypes={nodeTypes}
        onNodesChange={useStore((st) => st.onNodesChange)} onEdgesChange={useStore((st) => st.onEdgesChange)}
        onConnect={useStore((st) => st.onConnect)} onConnectEnd={onConnectEnd} isValidConnection={isValidConnection}
        onConnectStart={(_, p) => s().setConnecting(p.nodeId && p.handleId && p.handleType ? { nodeId: p.nodeId, handleId: p.handleId, handleType: p.handleType } : null)}
        onReconnect={onReconnect} onReconnectStart={onReconnectStart} onReconnectEnd={onReconnectEnd} edgesReconnectable reconnectRadius={18}
        connectionLineComponent={ConnectionLine}
        onNodeClick={(_, n) => s().select(n.id)}
        onInit={() => setReady(true)}
        onMoveEnd={(_, v) => s().setView(v)}
        onNodeDoubleClick={(_, n) => { if (n.data.kind !== "reroute") s().setRenaming(n.id); }}
        onPaneClick={(e) => {
          if (placing) { s().addNodeAt(placing, snap({ ...flowAt(e.clientX, e.clientY), })); return; }
          s().deselectAll();
        }}
        onPaneContextMenu={(e) => { e.preventDefault(); const me = e as unknown as MouseEvent; setMenu({ kind: "pane", x: me.clientX, y: me.clientY, flow: flowAt(me.clientX, me.clientY) }); }}
        onNodeContextMenu={(e, n) => { e.preventDefault(); s().select(n.id); setMenu({ kind: "node", x: e.clientX, y: e.clientY, id: n.id }); }}
        onEdgeContextMenu={(e, ed) => { e.preventDefault(); setMenu({ kind: "edge", x: e.clientX, y: e.clientY, id: ed.id, flow: flowAt(e.clientX, e.clientY) }); }}
        ...
        onDrop={(e) => { e.preventDefault(); const kind = e.dataTransfer.getData("application/covenants-kind"); if (kind) s().addNodeAt(kind, snap(flowAt(e.clientX, e.clientY))); }}
        onDragOver={(e) => { e.preventDefault(); e.dataTransfer.dropEffect = "move"; }}
        panOnDrag={[1, 2]} panActivationKeyCode="Space" selectionOnDrag selectionMode={SelectionMode.Partial} multiSelectionKeyCode="Shift"
        zoomOnScroll zoomOnPinch panOnScroll={false} zoomOnDoubleClick={false} preventScrolling
        snapToGrid snapGrid={[GRID, GRID]}
        minZoom={0.2} maxZoom={2} deleteKeyCode={null} selectionKeyCode={null}
      ...
      >
        ...
        {showMinimap && <MiniMap pannable zoomable nodeColor={(n) => n.type === "comment" ? "#E4E7EC" : "#C9D0D9"} maskColor="rgba(244,245,247,.72)" />}
      </ReactFlow>
      ...
      <div className="zoom-readout" title="scroll to zoom · F frames the selection · Home frames all">{Math.round(zoom * 100)}%</div>
      ...
      {menu?.kind === "add" && (
        <AddMenu x={menu.x} y={menu.y} pendingPort={pendingPort} pendingSide={menu.pending?.handleType}
          onPick={(kind) => {
      ...
      {menu?.kind === "node" && <ContextMenu x={menu.x} y={menu.y} items={nodeMenu(menu.id)} onClose={() => setMenu(null)} />}
      {menu?.kind === "edge" && <ContextMenu x={menu.x} y={menu.y} items={edgeMenu(menu.id, menu.flow)} onClose={() => setMenu(null)} />}
      {menu?.kind === "pane" && <ContextMenu x={menu.x} y={menu.y} items={paneMenu(menu.x, menu.y, menu.flow)} onClose={() => setMenu(null)} />}
      {pinMenu && <ContextMenu x={pinMenu.x} y={pinMenu.y} items={pinMenuItems(pinMenu)} onClose={() => s().setPinMenu(null)} />}
    </div>
  );
}
