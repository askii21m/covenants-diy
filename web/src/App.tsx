...
function useOpeners() {
  ...
  const openFile = useCallback(async (file: File | undefined) => {
    ...
    try { raw = JSON.parse(await file.text()); parsed = sanitizeFlow(raw); } catch { return false; }
    if (!parsed) return false;
    newDoc(parsed.name || file.name.replace(/\.covenants\.json$|\.json$/i, "") || "untitled", parsed);
    const v = (raw as FlowFile).view;
    if (v && [v.x, v.y, v.zoom].every(Number.isFinite) && v.zoom > 0) useStore.getState().setView({ x: v.x, y: v.y, zoom: v.zoom });
    return true;
  }, [newDoc]);
...
function exportJson() {
  ...
  a.href = URL.createObjectURL(blob); a.download = `${name.replace(/[^\w.-]+/g, "_")}.covenants.json`; a.click();
...
function TopBar({ openMenu, open }: { openMenu: OpenMenu; open: MenuName | null }) {
  ...
  const refs = { file: useRef<HTMLButtonElement>(null), edit: useRef<HTMLButtonElement>(null), view: useRef<HTMLButtonElement>(null) };
  return (
    <header className="top">
      ...
      <nav className="acts" role="menubar">
        {(["file", "edit", "view"] as const).map((which) => (
          <button key={which} ref={refs[which]} role="menuitem" aria-haspopup="menu" aria-expanded={open === which}
            ...
            onMouseDown={(e) => { e.preventDefault(); openMenu(which, refs[which].current!); }}
            onMouseEnter={() => { if (open && open !== which) openMenu(which, refs[which].current!); }}>
            {which}
          </button>
        ))}
      </nav>
      <div className="sp" />
      <label className="sel"><span>network</span>
        <select value={network} onChange={(e) => setNetwork(e.target.value as (typeof NETWORKS)[number])}>
          {NETWORKS.map((n) => <option key={n} value={n}>{n}</option>)}
        </select>
      </label>
      <label className="sel"><span>assume active</span>
        <select value={ruleset} onChange={(e) => setRuleset(e.target.value)}>
          {Object.entries(RULESETS).map(([k, v]) => <option key={k} value={k} title={v.hint}>{v.label}</option>)}
        </select>
      </label>
      <a className="gh" href="https://github.com/askii21m/covenants-diy" target="_blank" rel="noreferrer noopener"
...
function Tabs({ openMenu }: { openMenu: OpenMenu }) {
  ...
  useEffect(() => { strip.current?.querySelector(".tab.on")?.scrollIntoView({ inline: "nearest", block: "nearest" }); }, [active, docs.length]);
  return (
    <div className="tabs" role="tablist">
      <div className="tab-scroll" ref={strip}>
        {docs.map((d) => (
          <div key={d.id} role="tab" aria-selected={d.id === active} className={`tab ${d.id === active ? "on" : ""}`}
            onClick={() => switchDoc(d.id)}
            onMouseDown={(e) => { if (e.button === 1) { e.preventDefault(); closeDoc(d.id); } }}>
            <InlineName className="tab-name" value={d.name} placeholder="untitled" editing={editing === d.id}
              onStart={() => setEditing(d.id)} onCommit={(v) => { if (v) renameDoc(d.id, v); setEditing(null); }} />
            <button className="tab-x" aria-label={`close ${d.name}`} title="close"
              onClick={(e) => {
                e.stopPropagation();
      ...
      <button ref={add} className="tab-add" aria-label="new document" title="new" onMouseDown={(e) => { e.preventDefault(); openMenu("new", add.current!); }}>+</button>
    </div>
  );
}
...
function SaveBanner() {
  ...
  return (
    <div className="banner" role="alert">
      <span>Autosave failed: the browser refused the write, most likely because storage is full. Your work is only in this tab. Export what you want to keep, or close documents you no longer need.</span>
      <button className="banner-x" aria-label="dismiss" title="dismiss" onClick={() => setHidden(true)}>×</button>
    </div>
  );
}
...
function Divider() {
  ...
  return (
    <div className="div" role="separator" aria-orientation="horizontal" title="drag to resize"
      onPointerDown={(e) => { dragging.current = true; (e.target as HTMLElement).setPointerCapture(e.pointerId); }}
      onPointerMove={(e) => { if (dragging.current) setPanelHeight(window.innerHeight - e.clientY); }}
...
function Boot({ openExample, say }: { openExample: (key: string) => void; say: (t: string) => void }) {
  ...
  const openShared = useCallback(async (shared: SharedDoc | null) => {
  ...
  useEffect(() => {
    ...
    const id = idOnUrl();
    if (id) { void fetchShared(id).then(openShared).then((ok) => { if (!ok) openExample("vault"); }); return; }
    const fragment = fragmentOnUrl();
    if (fragment) { void decodeFlow(fragment).then(openShared).then((ok) => { if (!ok) openExample("vault"); }); return; }
  ...
  useEffect(() => {
    const onHash = () => { const f = fragmentOnUrl(); if (f) void decodeFlow(f).then(openShared); };
...
export default function App() {
  ...
  const openMenu = useCallback<OpenMenu>((which, trigger) => {
    ...
    setMenu((m) => (m && m.which === which && m.trigger === trigger ? null : { which, x: r.left, y: r.bottom + 4, trigger }));
  }, []);
  const copyLink = useCallback(() => {
    ...
    const written = typeof ClipboardItem === "function" && navigator.clipboard?.write
  ...
  const newItems: MenuItem[] = [
    ...
    ...EXAMPLE_GROUPS.flatMap(({ title, keys }) => [
      ...
      ...keys.map((k) => ({ label: EXAMPLES[k].label, detail: EXAMPLES[k].blurb, shortcut: EXAMPLES[k].needs, onClick: () => openExample(k) })),
    ]),
  ];
  ...
  const editItems: MenuItem[] = [
    ...
    { label: "Comment around selection", shortcut: "C", onClick: () => useStore.getState().commentAroundSelection(), disabled: !hasSelection },
  ];
  ...
  return (
    <ReactFlowProvider>
      <div className="app" style={{ ["--panel" as string]: `${panelHeight}px` }}>
        ...
        {menu && <ContextMenu x={menu.x} y={menu.y} items={menu.which === "file" ? fileItems : menu.which === "edit" ? editItems : menu.which === "view" ? viewItems : newItems} ignore={menu.trigger} onClose={() => setMenu(null)} />}
        <input ref={file} type="file" accept="application/json,.json" hidden
          onChange={async (e) => { const f = e.target.files?.[0]; e.target.value = ""; const ok = await openFile(f); if (ok === false) say("That file is not a covenants flow."); }} />
        {toast && <div className="toast" role="alert" key={toast.n}>{toast.text}</div>}
