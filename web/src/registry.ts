...
export type PortType = "hash" | "script" | "tx" | "spk" | "hex" | "outpoint" | "witness" | "number" | "text" | "address" | "any";
...
export const I32_MIN = -2_147_483_648, I32_MAX = 2_147_483_647;
...
const N = (id: string, label: string, min: number, max: number): Port =>
  ({ id, label, type: "number", field: "number", min, max });
...
export const COUNTS: Record<string, { min: number; max: number }> = {
  nIn: { min: 1, max: 32 }, nOut: { min: 1, max: 32 },
  nLeaves: { min: 1, max: 16 }, nItems: { min: 0, max: 32 }, nParts: { min: 1, max: 16 },
};
...
const template: NodeKind = {
  ...
  label: "Template",
  category: "Covenants",
  description: "A transaction without prevouts or witnesses: what a covenant commits to. Emits the BIP-119 hash per input.",
  defaults: () => ({
    name: "template", version: 2, locktime: 0,
    nIn: 1, nOut: 1,
    in0_seq: 0xfffffffd, out0_value: 10_000, out0_spk: "",
  }),
  inputs: (f) => {
  ...
  compute: (f, w) => {
    const nIn = count(f, "nIn", 1), nOut = count(f, "nOut", 1);
    const inputs = Array.from({ length: nIn }, (_, i) => ({ sequence: num(get(f, w, `in${i}_seq`), 0xffffffff) >>> 0 }));
    const outputs = Array.from({ length: nOut }, (_, j) => ({ value: num(get(f, w, `out${j}_value`)), script_pubkey: str(get(f, w, `out${j}_spk`)) }));
    if (outputs.some((o) => !o.script_pubkey)) return fail("every output needs a scriptPubKey");
    try {
      const v = wasm.template({ version: num(get(f, w, "version"), 2), locktime: num(get(f, w, "locktime")), inputs, outputs });
...
const tapscript: NodeKind = {
  ...
  compute: (f, w, ctx) => {
    ...
    for (const [k, v] of Object.entries(w)) if (k.startsWith("ref_") && v != null && v !== "") bindings[k.slice(4)] = str(v);
    try {
      const v = wasm.assemble({ source: String(get(f, w, "source") ?? ""), bindings, ruleset: ctx.ruleset });
      if (v.error) return { outputs: { script: null, leaf_hash: null }, status: "error", message: `line ${v.error.line + 1}: ${v.error.message}`, extra: v };
      const enf = v.enforcement!;
      return {
        outputs: { script: v.script!, leaf_hash: v.leaf_hash! },
        status: enf.status === "enforced" ? "ok" : "warn",
        message: enf.status === "enforced" ? `${v.script!.length / 2} B · enforced` : `${enf.status}: ${enf.inactive.join(", ")} inactive`,
        extra: v,
      };
    } catch (e) { return fail(caught(e), { script: null, leaf_hash: null }); }
  },
};
...
const taproot: NodeKind = {
  ...
  outputs: (f) => {
    const ports: Port[] = [
      P("address", "address", "address"), P("spk", "scriptPubKey", "spk"),
      P("output_key", "output key", "hash"), P("merkle_root", "merkle root", "hash"),
    ];
  ...
  compute: (f, w, ctx) => {
    ...
    try {
      ...
      const out: Record<string, Value> = { address: v.address, spk: v.script_pubkey, output_key: v.output_key, merkle_root: v.merkle_root ?? "" };
...
const transaction: NodeKind = {
  ...
  compute: (f, w) => {
    ...
    const values = Array.from({ length: nIn }, (_, i) => { const v = get(f, w, `value${i}`); return v == null || v === "" ? null : num(v); });
    try {
      const v = wasm.realize({ template: tpl, prevouts, witnesses, prevout_values: values as (number | null)[] as any });
...
const witness: NodeKind = {
  ...
  compute: (f, w) => {
    ...
    const script = str(w.script), control = str(w.control);
...
const outpoint: NodeKind = {
  ...
  inputs: () => [P("txid", "txid", "hash", "hex"), N("vout", "vout", 0, U32_MAX), N("value", "value sat", 0, MAX_MONEY)],
  ...
  compute: (f, w) => {
    ...
    return { outputs: { outpoint: `${txid.toLowerCase()}:${num(get(f, w, "vout"))}`, value: num(get(f, w, "value")) }, status: "ok" };
  },
};
...
const execute: NodeKind = {
  ...
  inputs: () => [
    P("script", "script", "script"), P("witness", "witness", "witness"), P("tx", "tx hex", "tx"),
    N("input_index", "input index", 0, U32_MAX), N("prevout_value", "prevout sat", 0, MAX_MONEY),
    P("prevout_spk", "prevout spk", "spk"), P("control", "control block", "hex"),
  ],
  ...
  compute: (f, w, ctx) => {
    ...
    const control = str(w.control) || (full.length >= 2 && full[full.length - 2] === script ? full[full.length - 1] : "");
    ...
    } catch (e) { return fail(caught(e), { ok: null, stack: null }); }
  },
};
...
const templateHash: NodeKind = {
  ...
  label: "Template Hash",
  category: "Covenants",
  description: "BIP-446: the hash OP_TEMPLATEHASH pushes. Commits to version, locktime, every sequence, every output, the annex and this input's index, and to nothing about the coins being spent, which is what makes a signature over it rebindable.",
  ...
  compute: (f, w) => {
    ...
    try {
      return { outputs: { hash: wasm.template_hash_446({ tx, input_index: num(get(f, w, "input_index")), annex: annex || undefined }) }, status: "ok" };
    } catch (e) { return fail(caught(e), { hash: null }); }
  },
};
...
const slice: NodeKind = {
  ...
  label: "Slice",
  category: "Bytes",
  description: "A run of bytes out of a longer string, by offset and length. What a script rebuilding its own signature message needs its witness to carry.",
  ...
  compute: (f, w) => {
    ...
    const offset = num(get(f, w, "offset")), length = num(get(f, w, "length"));
    if (offset < 0 || length < 0) return fail("offset and length cannot be negative", { hex: null, rest: null });
    if (offset + length > total) return fail(`wants bytes ${offset}..${offset + length} of only ${total}`, { hex: null, rest: null });
    return { outputs: { hex: data.slice(offset * 2, (offset + length) * 2), rest: data.slice((offset + length) * 2) }, status: "ok", message: `${length} of ${total} B` };
  },
};
...
const leBytes: NodeKind = {
  ...
  label: "Little-endian",
  category: "Bytes",
  description: "A number as fixed-width little-endian bytes, the way amounts, versions and sequences appear inside a signature message.",
  ...
  compute: (f, w) => {
    ...
    if (!Number.isSafeInteger(value) || value < 0) return fail("value must be a whole number, zero or more", { hex: null });
    let v = BigInt(value);
    const out: string[] = [];
    for (let i = 0; i < width; i++) { out.push(Number(v & 0xffn).toString(16).padStart(2, "0")); v >>= 8n; }
...
const text: NodeKind = {
  ...
  label: "Text",
  category: "Bytes",
  description: "UTF-8 text as bytes. What an oracle signs under BIP-348 is the message itself, not a hash of it, so the bytes are what goes on the stack.",
  ...
  compute: (f, w) => {
    ...
    return { outputs: { hex: Array.from(bytes, (b) => b.toString(16).padStart(2, "0")).join(""), length: bytes.length }, status: "ok" };
  },
};
...
const concat: NodeKind = {
  ...
  label: "Concat",
  category: "Bytes",
  description: "Joins byte strings, left to right. What OP_CAT does on the stack, for building a value the script will rebuild.",
...
const sha256: NodeKind = {
  ...
  compute: (f, w) => { try { return { outputs: { hash: wasm.sha256(str(get(f, w, "data"))) }, status: "ok" }; } catch (e) { return fail(caught(e), { hash: null }); } },
};
...
const taggedHash: NodeKind = {
  ...
  outputs: () => [P("hash", "hash", "hash")],
  compute: (f, w) => { try { return { outputs: { hash: wasm.tagged_hash(str(get(f, w, "tag")), str(get(f, w, "data"))) }, status: "ok" }; } catch (e) { return fail(caught(e), { hash: null }); } },
};
...
const key: NodeKind = {
  ...
  compute: (f, w) => {
    ...
    try { return { outputs: { pubkey: wasm.pubkey(secret), sk: secret }, status: "ok" }; }
    catch (e) { return fail(caught(e), { pubkey: null, sk: null }); }
  },
};
...
export const HASH_TYPES: Record<string, number> = {
  ...
  "ALL|ANYONECANPAY": 0x81, "NONE|ANYONECANPAY": 0x82, "SINGLE|ANYONECANPAY": 0x83,
  "ALL|ANYPREVOUT": 0x41, "NONE|ANYPREVOUT": 0x42, "SINGLE|ANYPREVOUT": 0x43,
  "ALL|ANYPREVOUTANYSCRIPT": 0xc1, "NONE|ANYPREVOUTANYSCRIPT": 0xc2, "SINGLE|ANYPREVOUTANYSCRIPT": 0xc3,
};
...
const sighash: NodeKind = {
  ...
  label: "Sighash",
  category: "Keys",
  description: "The BIP-341 script-path digest for one input of a transaction: what a signature checked in this leaf must sign. The ANYPREVOUT modes are BIP-118's.",
  defaults: () => ({ name: "sighash", hash_type: "DEFAULT", input_index: 0, prevout_value: 0, prevout_spk: "" }),
  inputs: () => [
    P("tx", "tx", "tx"), P("leaf", "leaf script", "script"),
    { ...P("hash_type", "hash type", "text", "select"), options: Object.keys(HASH_TYPES) },
    N("input_index", "input index", 0, U32_MAX),
    N("prevout_value", "prevout sat", 0, MAX_MONEY), P("prevout_spk", "prevout spk", "spk", "hex"),
  ],
  outputs: () => [P("sighash", "sighash", "hash"), P("preimage", "preimage", "hex"), P("type_byte", "hash type byte", "number"), P("leaf_hash", "leaf hash", "hash")],
  compute: (f, w) => {
    const name = str(get(f, w, "hash_type")) || "DEFAULT";
    const hash_type = HASH_TYPES[name];
    const tx = str(get(f, w, "tx")), leaf = str(get(f, w, "leaf"));
    const spk = str(get(f, w, "prevout_spk")), value = num(get(f, w, "prevout_value"));
    ...
    try {
      const v = wasm.sighash({ tx, input_index: num(get(f, w, "input_index")), prevouts: spk ? [{ value, script_pubkey: spk }] : [], hash_type, leaf_script: leaf });
      return { outputs: { sighash: v.sighash, preimage: v.preimage, type_byte: v.hash_type, leaf_hash: v.tapleaf_hash }, status: "ok", message: v.key_version === 1 ? "BIP-118 key version" : undefined };
    } catch (e) { return fail(caught(e), empty); }
  },
};
...
const sign: NodeKind = {
  ...
  label: "Schnorr Sign",
  category: "Keys",
  description: "A BIP-340 signature over a 32-byte message. For a CHECKSIG, wire the sighash and its type byte: the byte is appended unless it is DEFAULT. For CHECKSIGFROMSTACK use the raw 64 bytes.",
  defaults: () => ({ name: "sign", message: "", hash_type: 0 }),
  inputs: () => [P("secret", "secret", "hex", "hex"), P("message", "message", "hash", "hex"), N("hash_type", "hash type byte", 0, 255)],
  outputs: () => [P("sig", "signature", "hex"), P("raw", "raw 64 bytes", "hex")],
  compute: (f, w) => {
    const secret = str(get(f, w, "secret")), message = str(get(f, w, "message")), t = num(get(f, w, "hash_type"));
    ...
    } catch (e) { return fail(caught(e), { sig: null, raw: null }); }
  },
};
...
const verify: NodeKind = {
  ...
  inputs: () => [P("pubkey", "public key", "hex", "hex"), P("message", "message", "hash", "hex"), P("signature", "signature", "hex", "hex")],
  outputs: () => [P("ok", "valid", "number")],
  compute: (f, w) => {
    const pk = str(get(f, w, "pubkey")), message = str(get(f, w, "message")), sig = str(get(f, w, "signature"));
    ...
    } catch (e) { return fail(caught(e), { ok: null }); }
  },
};

export const KINDS: Record<string, NodeKind> = Object.fromEntries(
  [template, tapscript, taproot, execute, transaction, witness, outpoint, input, key, sighash, sign, verify, templateHash, slice, leBytes, text, concat, sha256, taggedHash, reroute, comment].map((k) => [k.kind, k]),
);
