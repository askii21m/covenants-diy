...
function Field({ port, value, onChange }: { port: Port; value: unknown; onChange: (v: unknown) => void }) {
  ...
  const commit = (v: string) => {
    ...
    timer.current = setTimeout(() => {
      ...
      if (useStore.getState().active === doc) onChange(parse(v));
    }, 180);
  };
  const flush = () => {
  ...
  return (
    <input
      ...
      min={port.min}
      max={port.max}
      step={port.field === "number" ? 1 : undefined}
      title={
        port.field === "number" && (port.min != null || port.max != null)
          ? `${port.min?.toLocaleString("en-US") ?? "any"} to ${port.max?.toLocaleString("en-US") ?? "any"}`
          : undefined
      }
      inputMode={port.field === "number" ? "numeric" : undefined}
      spellCheck={false}
      onChange={(e) => {
...
function CovNodeImpl({ id, data, selected }: NodeProps<FlowNode>) {
  ...
  for (const p of inputs)
    if (p.id.startsWith("ref_")) {
      const v = wiredValue(p.id);
      if (v != null) refsBound[p.id.slice(4)] = String(v);
    }
  ...
  const alt = (handleId: string) => (e: React.PointerEvent | React.MouseEvent) => {
    if (e.altKey && e.button === 0) {
      e.preventDefault();
      e.stopPropagation();
      breakWires(id, handleId);
    }
  };
  const pinProps = (handleId: string, side: "source" | "target") => ({
    onContextMenu: (e: React.MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
      setPinMenu({ nodeId: id, handleId, side, x: e.clientX, y: e.clientY });
    },
    onPointerDownCapture: alt(handleId),
    onMouseDownCapture: alt(handleId),
  });

  return (
    <div
      className={`cov k-${kind.category.toLowerCase()} ${selected ? "on" : ""} st-${status} ${collapsed ? "collapsed" : ""}`}
    >
      <div className="hd">
        <button
          className="chev nodrag"
          title={collapsed ? "expand" : "collapse"}
          onClick={(e) => {
            e.stopPropagation();
            setField(id, "collapsed", !collapsed);
          }}
        >
          {collapsed ? "▸" : "▾"}
        </button>
        ...
        <InlineName
          ...
          placeholder={kind.label.toLowerCase()}
          editing={renaming}
          onStart={() => setRenaming(id)}
          onCommit={(v) => {
            if (v != null && v !== "") setField(id, "name", v);
            setRenaming(null);
          }}
        />
        ...
        <div className="stubs">
          ...
          {outputs.map((p) => (
            <Handle
              ...
              id={p.id}
              className={`p t-${p.type}${dim(p, "source")}`}
              title={`${p.label} · ${p.type}`}
              {...pinProps(p.id, "source")}
            />
          ))}
        </div>
      )}
        ...
        {inputs.map((p) =>
          p.wide ? (
            <div className="r wide" key={p.id}>
              ...
              <pre className={`src ${scriptBlock(String(data[p.id] ?? "")).clipped ? "clipped" : ""}`}>
                {highlight(String(data[p.id] ?? ""), undefined, refsBound)}
              </pre>
            </div>
          ) : (
            <div className="r in" key={p.id}>
              ...
              {(() => {
                ...
                if (w !== undefined)
                  return (
                    <span className="f wired" title={Array.isArray(w) ? w.join("\n") : String(w)}>
                      {p.label.includes("sat") ? sats(w) : short(w, 16)}
                    </span>
                  );
                if (!p.field)
                  return (
                ...
                if (p.field === "select")
                  return (
                    <select
                      className="f sel nodrag nopan"
                      value={String(data[p.id] ?? p.options?.[0] ?? "")}
                      aria-label={p.label}
                      onChange={(e) => setField(id, p.id, e.target.value)}
                      onMouseDown={(e) => e.stopPropagation()}
                    >
                      {(p.options ?? []).map((o) => (
        ...
        {counts.map(([key, label, min]) => (
          <span className="cnt" key={key}>
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                setField(id, key, Math.max(min, Number(data[key]) - 1));
              }}
            >
              -
            </button>
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                setField(id, key, Number(data[key]) + 1);
              }}
            >
              +
            </button>
