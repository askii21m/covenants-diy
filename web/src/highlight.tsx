...
export function highlight(source: string, err?: SourceError, bound?: Record<string, string>): ReactNode[] {
  ...
  source.split("\n").forEach((line, li) => {
    ...
    while ((m = re.exec(code))) {
      ...
      let cls = word.startsWith("@")
        ? "ref"
        : /^(OP_)?[A-Z][A-Z0-9_]+$/.test(word)
          ? "op"
          : /^-?\d+$|^<[0-9a-fA-F]*>$|^(0x)?[0-9a-fA-F]{2,}$/.test(word)
            ? "num"
            : "";
      ...
      const title = bad
        ...
        : word.startsWith("@")
          ? bound?.[word.slice(1)]
            ? `${word} = ${bound[word.slice(1)]}`
            : `${word}: nothing wired`
          : undefined;
      out.push(
        <span key={`${li}-${wi}`} className={cls.trim() || undefined} title={title}>
          {word}
        </span>,
      );
      wi++;
