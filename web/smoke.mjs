...
const hot = assemble({
  ...
  bindings: { withdraw: withdraw.ctv[0] },
});
...
const trigger = template({
  ...
  outputs: [{ value: 99_000, script_pubkey: trig.script_pubkey }],
});
...
const triggerTx = realize({
  template: trigger.template,
  prevouts: [funding],
  witnesses: [[depLeaf.script, deposit.control_blocks[0]]],
  prevout_values: [100_000],
});
check("trigger tx is complete", triggerTx.complete, true);
check("fee is computed", triggerTx.fee, 1000);
const withdrawTx = realize({
  template: withdraw.template,
  prevouts: [triggerTx.outpoints[0]],
  witnesses: [[hot.script, trig.control_blocks[0]]],
});
...
const run = execute({
  script: hot.script,
  tx: withdrawTx.hex,
  prevouts: [{ value: 99_000, script_pubkey: trig.script_pubkey }],
  control_block: trig.control_blocks[0],
});
...
const earlyTx = realize({
  template: early.template,
  prevouts: [triggerTx.outpoints[0]],
  witnesses: [[hot.script, trig.control_blocks[0]]],
});
const runEarly = execute({
  script: hot.script,
  tx: earlyTx.hex,
  prevouts: [{ value: 99_000, script_pubkey: trig.script_pubkey }],
});
...
const alice = "1".repeat(64),
  oracle = "2".repeat(64);
const alicePk = pubkey(alice),
  oraclePk = pubkey(oracle);
...
const spendTx = realize({
  template: spend.template,
  prevouts: [funding],
  witnesses: [[sigLeaf.script, sigOut.control_blocks[0]]],
  prevout_values: [100_000],
});
...
const runSig = execute({
  script: sigLeaf.script,
  stack: [aliceSig],
  tx: spendTx.hex,
  prevouts: prev,
  control_block: sigOut.control_blocks[0],
});
check("CHECKSIG passes with the computed sighash", runSig.success, true);
const runBad = execute({
  script: sigLeaf.script,
  stack: [sign_schnorr(oracle, sh.sighash)],
  tx: spendTx.hex,
  prevouts: prev,
});
...
const apoTx = realize({
  template: spend.template,
  prevouts: [funding],
  witnesses: [[apoLeaf.script, apoOut.control_blocks[0]]],
  prevout_values: [100_000],
});
...
const apoSh = sighash({
  ...
  prevouts: apoPrev,
  hash_type: 0x41,
  leaf_script: apoLeaf.script,
});
check("ANYPREVOUT sighash uses key version 1", apoSh.key_version, 1);
const apoSig = sign_schnorr(alice, apoSh.sighash) + "41";
const runApo = execute({
  script: apoLeaf.script,
  stack: [apoSig],
  tx: apoTx.hex,
  prevouts: apoPrev,
  control_block: apoOut.control_blocks[0],
  ruleset: { ctv: true, csfs: true, cat: false, apo: true },
});
...
const otherTx = realize({
  template: spend.template,
  prevouts: ["e".repeat(64) + ":0"],
  witnesses: [[apoLeaf.script, apoOut.control_blocks[0]]],
  prevout_values: [100_000],
});
const runRebound = execute({
  script: apoLeaf.script,
  stack: [apoSig],
  tx: otherTx.hex,
  prevouts: apoPrev,
  internal_key: alicePk,
  control_block: apoOut.control_blocks[0],
  ruleset: { ctv: true, csfs: true, cat: false, apo: true },
});
...
const runTextBad = execute({
  script: attest.script,
  stack: [textSig, Buffer.from("BTC-USD above 100001 on 2026-12-31", "utf8").toString("hex")],
  tx: spendTx.hex,
  prevouts: prev,
});
