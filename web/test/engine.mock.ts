...
const throwing = new Proxy({}, { get: () => () => { throw new Error("wasm is not available in tests"); } });
export const wasm = throwing as never;
export const RULESETS: Record<string, { label: string; flags: { ctv: boolean; csfs: boolean; cat: boolean; apo: boolean } }> = {
