...
describe("sanitizeFlow", () => {
  ...
  it("drops malformed nodes and edges to missing nodes, keeps the rest", () => {
    const out = sanitizeFlow({
      nodes: [
        good,
        { id: "b" },
        { id: "c", position: { x: "nan", y: 0 }, data: { kind: "comment" } },
        { id: "d", position: { x: 0, y: 0 }, data: { kind: "nope" } },
        { id: "a", position: { x: 9, y: 9 }, data: { kind: "comment" } },
        7,
        null,
      ],
  ...
  it("does not carry React Flow's runtime fields", () => {
    const out = sanitizeFlow({
      nodes: [{ ...good, measured: { width: 10, height: 10 }, selected: true, dragging: true }],
    })!;
