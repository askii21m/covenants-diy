...
import { KINDS, outOfRange, count, COUNTS, MAX_MONEY, U32_MAX, I32_MAX, type NodeFields, type Value } from "../src/registry";
import { sanitizeFlow } from "../src/store";
import { NETWORKS } from "../src/engine";

const ctx = { network: "signet" as const, ruleset: { ctv: true, csfs: true, cat: true, apo: true, templatehash: true, internalkey: true } };
...
beforeAll(async () => {
  ...
  const tpl = wasm.template({ inputs: [{ sequence: 0xfffffffd }], outputs: [{ value: 99_000, script_pubkey: "5120" + "11".repeat(32) }] });
...
function inputsFor(kind: string): Record<string, Value> {
  switch (kind) {
    case "template": return { out0_spk: "5120" + "11".repeat(32) };
    case "transaction": return { template: fixture.template, prevout0: "f".repeat(64) + ":0" };
    case "execute": return { script: fixture.leaf, tx: fixture.tx, prevout_spk: fixture.spk };
    case "sighash": return { tx: fixture.tx, leaf: fixture.leaf, prevout_spk: fixture.spk };
    case "template_hash": return { tx: fixture.tx };
    case "slice": return { data: "00112233" };
    case "sign": return { secret: "22".repeat(32), message: "ab".repeat(32) };
    case "witness": return { script: fixture.leaf, control: fixture.control };
    case "concat": return { part0: "aa", part1: "bb" };
    case "taproot": return { leaf0: fixture.leaf };
    case "key": return { secret: "22".repeat(32) };
    default: return {};
  }
}
...
function numericPorts(): Array<{ kind: string; id: string; label: string; min: number; max: number }> {
  ...
  for (const K of Object.values(KINDS)) {
    for (const p of K.inputs({ ...K.defaults(), kind: K.kind } as NodeFields)) {
      ...
      out.push({ kind: K.kind, id: p.id, label: p.label, min: p.min ?? Number.NEGATIVE_INFINITY, max: p.max ?? Number.POSITIVE_INFINITY });
    }
  }
  return out;
}
  ...
  it("refuses anything outside the range, rather than quietly using something else", () => {
    ...
    for (const p of numericPorts()) {
      ...
      for (const bad of [p.min - 1, p.max + 1, p.min + 0.5]) {
        ...
        if (r.status !== "error") accepted.push(`${p.kind}.${p.id} took ${bad} -> ${JSON.stringify(r.outputs).slice(0, 60)}`);
      }
    }
...
describe("counts", () => {
  ...
  it("draw the ports they compute with", () => {
    for (const [kind, key] of [["witness", "nItems"], ["concat", "nParts"], ["taproot", "nLeaves"], ["template", "nIn"], ["template", "nOut"], ["transaction", "nIn"]] as const) {
      const K = KINDS[kind];
      for (const bad of [-1, 0, 1.5, 9_999]) {
        ...
        const drawn = K.inputs(data).filter((p) => new RegExp(`^(item|part|leaf|in\\d|out\\d|prevout|witness|value)`).test(p.id));
        ...
        if (r.status === "error") expect(String(r.message), `${kind}.${key}=${bad}`).not.toMatch(/cannot be (below|above)/);
      }
    }
  });
});
